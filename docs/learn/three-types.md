...
For a practical example of what multi-level security looks like when applied to data rather than systems, see [Data-Centric Security on AWS](https://iamtgray.github.io/dcs-data-exchange/) -- which covers how security labels, attribute-based access control, and encryption can be embedded directly into data so that protection travels with it regardless of where it's stored or processed.
